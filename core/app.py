...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new_user:
        # РАЗВЕРНУТОЕ приветствие для НОВЫХ пользователей
        welcome_text = f"{greeting}, {user_name}! 👋\n\n"
        welcome_text += "🎯 <b>ОНП-СуперБот — твой личный репетитор ЕГЭ по обществознанию</b>\n\n"
        
        welcome_text += "✨ <b>Что умеет этот бот:</b>\n\n"
        
        welcome_text += "🆓 <b>Тестовая часть БЕСПЛАТНО:</b>\n"
        welcome_text += "• 1000+ вопросов с подробными разборами\n"
        welcome_text += "• Все темы кодификатора ФИПИ\n"
        welcome_text += "• Режим экзамена (задания 1-16)\n"
        welcome_text += "• Тренировка по блокам и номерам\n"
        welcome_text += "• Работа над ошибками\n\n"
        
        if not subscription_info or not subscription_info.get('is_active'):
            welcome_text += "💎 <b>Premium-модули (от 199₽/мес):</b>\n\n"
            
            welcome_text += "🤖 <b>ИИ-проверка за секунды</b>\n"
            welcome_text += "Специально обученная нейросеть проверяет развёрнутые ответы по критериям ФИПИ. "
            welcome_text += "Подробный разбор как от эксперта!\n\n"
            
            welcome_text += "📝 <b>Задания второй части (19-25)</b>\n"
            welcome_text += "• Задание 19 — Анализ суждений\n"
            welcome_text += "• Задание 20 — Работа с текстом\n"
            welcome_text += "• Задание 24 — Составление плана\n"
            welcome_text += "• Задание 25 — Эссе и обоснования\n\n"
            
            welcome_text += "📊 <b>Продвинутая аналитика</b>\n"
            welcome_text += "Детальная статистика, персональные рекомендации, прогноз баллов\n\n"

            # Информация о бесплатных проверках
            welcome_text += "🎁 <b>Специальное предложение:</b>\n"
            welcome_text += "• 3 бесплатных AI-проверки каждый день\n"
            welcome_text += "• Попробуй задания 19-25 без подписки\n"
            welcome_text += "• Лимит обновляется ежедневно\n\n"

        welcome_text += "👇 <b>Выбери модуль и начни подготовку!</b>"
        ...
        if subscription_info and subscription_info.get('is_active'):
            ...
            welcome_text += "• От 199₽/месяц за модуль\n\n"
