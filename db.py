...
async def update_correct_streak(user_id: int) -> Tuple[int, int]:
    ...
    async with aiosqlite.connect(DATABASE_FILE) as db:
        ...
        row = await cursor.fetchone()
        db_current_correct, db_max_correct = 0, 0 # Инициализация на случай, если row is None
        if row:
            db_current_correct = row[0] or 0
            db_max_correct = row[1] or 0

        current_correct_streak = db_current_correct + 1 # Увеличиваем текущий стрик
        logger.info(f"Correct answer streak continued for user {user_id}. New correct streak: {current_correct_streak}")

        # Обновляем максимальный стрик
        max_correct_streak = db_max_correct
        if current_correct_streak > max_correct_streak:
            max_correct_streak = current_correct_streak
        ...
        try:
            ...
            await db.commit()
             ...
             return db_current_correct or 0, db_max_correct or 0
...
async def reset_correct_streak(user_id: int) -> None:
    """
    Сбрасывает стрик ПРАВИЛЬНЫХ ОТВЕТОВ ПОДРЯД до 0.
