...
Ответ оценивается по ДВУМ элементам:
  Элемент 1: Признаки понятия.
             Требуемое количество признаков определяется заданием (2 или 3).
             - Если требуется ≥3 признаков: минимум 2 из каталога, без неверных.
             - Если требуется ≥2 признаков: минимум 1 из каталога, без неверных.
...
СУТЬ ЗАДАНИЯ 18:
Ученику дан текстовый фрагмент (текст-источник) и УСЛОВИЕ ЗАДАНИЯ.
В УСЛОВИИ ЗАДАНИЯ указано КЛЮЧЕВОЕ ПОНЯТИЕ, для которого нужно
привести признаки/характеристики. Задание оценивается по ДВУМ ЭЛЕМЕНТАМ.

КРИТИЧЕСКИ ВАЖНО: Ключевое понятие берётся из УСЛОВИЯ ЗАДАНИЯ (вопроса),
а НЕ из текста-источника. Текст-источник нужен только для Элемента 2.
Например, если условие просит «характеристики государственного бюджета»,
а текст-источник рассказывает о социальной политике и доходах населения —
ключевое понятие всё равно «государственный бюджет», а не «социальная
политика» или «доходы населения».

ЭЛЕМЕНТ 1 — Признаки/характеристики КЛЮЧЕВОГО ПОНЯТИЯ (из условия задания):
  Ученик должен привести признаки/характеристики понятия, указанного
  в УСЛОВИИ ЗАДАНИЯ. Эти признаки проверяются по КАТАЛОГУ ПРИЗНАКОВ
  для данного понятия (если каталог предоставлен).
  Количество требуемых признаков определяется условием задания (2 или 3).
  - Если требуется ≥3: минимум 2 должны соответствовать каталогу.
  - Если требуется ≥2: минимум 1 должен соответствовать каталогу.
  Наличие хотя бы одного НЕВЕРНОГО признака обнуляет весь элемент 1.

ЭЛЕМЕНТ 2 — Объяснение связи с текстом-источником:
  Ученик должен объяснить связь между конкретной характеристикой/функцией,
  упомянутой автором в ТЕКСТЕ-ИСТОЧНИКЕ, и более широким явлением.
  Требуется каузальная или функциональная связь, а не простое цитирование.
  Ученику достаточно опираться на текст-источник.

КРИТЕРИИ ОЦЕНИВАНИЯ (максимум 2 балла):
• 2 балла — оба элемента верны (достаточное число признаков +
...
1. КОЛИЧЕСТВО ПРИЗНАКОВ ЗАВИСИТ ОТ ЗАДАНИЯ.
   Смотри поле «ТРЕБУЕМОЕ КОЛИЧЕСТВО» — оно может быть 2 или 3.
   - Если требуется ≥3: минимум 2 должны быть из каталога.
   - Если требуется ≥2: минимум 1 должен быть из каталога,
     а второй — НЕ неверным (т.е. верным или допустимым).

2. НЕ ЗАСЧИТЫВАЙ признаки, которые уже содержатся в ФОРМУЛИРОВКЕ
...
3. НЕ ЗАСЧИТЫВАЙ признаки, сформулированные через ОТРИЦАНИЕ
   («не является коммерческим» и т.п.).

4. НЕ ЗАСЧИТЫВАЙ признаки, определённые через ЭТИМОЛОГИЮ СЛОВА,
   МЕТАФОРУ или АЛЛЕГОРИЮ.

5. НЕ ЗАСЧИТЫВАЙ «ПУСТЫЕ» признаки — общие слова без содержания
   («важный», «значимый» и т.п.).

6. ЕСЛИ ЕСТЬ ХОТЯ БЫ ОДНА НЕВЕРНАЯ ПОЗИЦИЯ — весь элемент 1 НЕ
   засчитывается, даже если остальные признаки верны.

7. ЗАСЧИТЫВАЙ близкие по смыслу формулировки — не требуй дословного
   совпадения с эталоном. Разные учебники дают разные определения.

8. Если один признак ДУБЛИРУЕТ другой по смыслу — засчитывается один.

9. Если ученик выписал определение без разбивки на признаки — проверь,
   можно ли выделить из него требуемое количество конкретных признаков.

ВАЖНЫЕ ПРАВИЛА ОЦЕНИВАНИЯ ЭЛЕМЕНТА 2 (Объяснение связи):

1. ПРОВЕРЯЙ ОПОРУ НА ТЕКСТ-ИСТОЧНИК. Ученик должен объяснить связь
   именно той функции/характеристики, которую упоминает АВТОР в
   ТЕКСТЕ-ИСТОЧНИКЕ, а не произвольной.
...
Отвечай ВСЕГДА в формате JSON."""
    ...
    def _build_evaluation_prompt(
        ...
        if has_source_text:
            text_section = f"ТЕКСТ (тот же, что и в задании 17):\n{text}"
            element2_instructions = """ШАГ 2: Проверка ЭЛЕМЕНТА 2 (Объяснение связи с текстом):
  a) Найди в ТЕКСТЕ-ИСТОЧНИКЕ, КАКАЯ конкретно характеристика/функция
     упоминается автором.
  b) Проверь, ссылается ли объяснение ученика именно на функцию/
     характеристику из текста-источника (не на произвольную).
  ...
  ВАЖНО: Ответ ученика на задание 17 НЕ требуется. Ученику достаточно
  опираться на текст-источник."""
  ...
  Установи element2_accepted = false и в комментарии укажи, что текст не предоставлен.
...
КЛЮЧЕВОЕ ПОНЯТИЕ (извлечено из УСЛОВИЯ ЗАДАНИЯ, а НЕ из текста-источника): {concept}

ВАЖНО: Признаки ученика проверяй ИМЕННО для понятия «{concept}»,
указанного в условии задания. НЕ путай его с другими понятиями,
которые могут упоминаться в тексте-источнике.
...
ШАГ 1: Проверка ЭЛЕМЕНТА 1 (Признаки понятия):
  ...
  e) Определи, засчитан ли элемент 1 в зависимости от ТРЕБУЕМОГО
     КОЛИЧЕСТВА:
     - Если требуется ≥3: верных ≥ 3 И минимум 2 из каталога →
       элемент 1 засчитан.
     - Если требуется ≥2: верных ≥ 2 И минимум 1 из каталога →
       элемент 1 засчитан.
...
{{
    ...
    "element2_evaluation": {{
        "text_reference": "какая функция/характеристика из текста-источника использована",
