...
ЭЛЕМЕНТ 1 — Признаки понятия:
  ...
  - Если требуется ≥3 признаков: минимум 2 должны соответствовать
    каталогу признаков (эталонному перечню).
  - Если требуется ≥2 признаков: минимум 1 должен соответствовать
    каталогу признаков, а второй не должен быть неверным.
  В обоих случаях наличие хотя бы одного неверного признака
  обнуляет весь элемент 1.

ЭЛЕМЕНТ 2 — Объяснение связи с текстом:
  Ученик должен объяснить связь между конкретной характеристикой/функцией,
  упомянутой автором в тексте-источнике, и более широким явлением. Требуется
  каузальная или функциональная связь, а не простое цитирование.
  Для проверки элемента 2 ученику достаточно опираться на текст-источник
  (тот же, что и в задании 17). Ответ ученика на задание 17 НЕ требуется.
