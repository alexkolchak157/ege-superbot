"""Обработчики для задания 18 ЕГЭ по обществознанию."""
...
# Глобальные данные (общий файл с task17)
passages_data: Dict[str, Any] = {}

# Evaluator
try:
    from .evaluator import Task18AIEvaluator

    evaluator = Task18AIEvaluator()
    EVALUATOR_AVAILABLE = True
except ImportError as e:
    logger.error(f"Failed to import task18 evaluator: {e}")
    evaluator = None
...
async def init_task18_data():
    """Загрузка текстовых отрывков (общий файл с task17)."""
    ...
    try:
        ...
        with open(json_path, "r", encoding="utf-8") as f:
            ...
            logger.info(f"Loaded {count} text passages for task18")
    except Exception as e:
        logger.error(f"Failed to load task18 data: {e}")
...
@safe_handler()
async def cmd_task18(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /task18."""
    await show_main_menu(update, context)
    return states.CHOOSING_MODE
...
<b>📝 Задание 18 — Объяснение понятия</b>

Вам будет дан текстовый фрагмент. Нужно объяснить обществоведческое
понятие из текста и/или привести примеры, признаки, функции,
<b>опираясь на собственные знания курса обществознания</b>.
...
• Всего текстов: {stats['total_tasks']}"""
    ...
    keyboard = [
        [InlineKeyboardButton("🎯 Решать задания", callback_data="t18_practice")],
        [InlineKeyboardButton("📊 Моя статистика", callback_data="t18_progress")],
...
async def practice_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбрать случайный текст и показать задание 18."""
    query = update.callback_query
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка ответа ученика на задание 18."""
    ...
    if EVALUATOR_AVAILABLE and evaluator:
        try:
            ...
            logger.error(f"Task18 evaluation error: {e}")
            score = 0
...
"""
    ...
    # Правильные ответы
    correct = task18.get("correct_answer", {}).get("elements", [])
    if correct:
    ...
    keyboard = [
        [InlineKeyboardButton("🔄 Новое задание", callback_data="t18_new")],
        [InlineKeyboardButton("📊 Моя статистика", callback_data="t18_progress")],
        [InlineKeyboardButton("◀️ Назад в меню", callback_data="t18_menu")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def my_progress(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"""<b>📊 Ваша статистика по заданию 18</b>
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Выход из задания 18.")
...
async def get_user_stats(user_id: int) -> Dict[str, Any]:
    try:
        ...
        cursor = await conn.execute(
            ...
            FROM task18_attempts WHERE user_id = ?
            """,
            (user_id,),
        )
        ...
        logger.error(f"Error getting task18 stats: {e}")
        total, avg = 0, 0.0
...
async def get_detailed_stats(user_id: int) -> Dict[str, int]:
    try:
        ...
        logger.error(f"Error getting task18 detailed stats: {e}")
...
async def save_attempt(user_id: int, passage_id: int, answer: str, score: int):
    try:
        ...
        await conn.execute(
            """
            CREATE TABLE IF NOT EXISTS task18_attempts (
        ...
        await conn.execute(
            """
            INSERT INTO task18_attempts (user_id, passage_id, answer, score, attempted_at)
            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
            """,
            (user_id, passage_id, answer, score),
        )
        await conn.commit()
        logger.info(f"Saved task18 attempt: user={user_id}, passage={passage_id}, score={score}")
    except Exception as e:
        logger.error(f"Error saving task18 attempt: {e}")
