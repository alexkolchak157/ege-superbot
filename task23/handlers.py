...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Подсчет вопросов по типам
    questions = task23_data.get('questions', [])
    type1_count = sum(1 for q in questions if q.get('model_type') == 1)
    type2_count = sum(1 for q in questions if q.get('model_type') == 2)

    instruction = task23_metadata.get('instruction', '')

    text = f"""{greeting}

<b>📜 Задание 23 - Конституция РФ</b>

На основе положений Конституции Российской Федерации сформулируйте подтверждения характеристик конституционно-правового статуса нашего государства.

<b>Типы заданий:</b>
• <b>Тип 1:</b> Одна характеристика → 3 подтверждения ({type1_count} вопросов)
• <b>Тип 2:</b> Три характеристики → по 1 подтверждению ({type2_count} вопросов)
...
• Всего заданий: {stats['total_tasks']}"""
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📖 Тип 1 (одна характеристика)", callback_data="t23_practice_type1")],
        [InlineKeyboardButton("📖 Тип 2 (три характеристики)", callback_data="t23_practice_type2")],
...
<b>📝 Формат ответа:</b>
Сформулируйте <b>3 подтверждения</b> этой характеристики на основе Конституции РФ.

Отправьте ответ в виде нумерованного списка:
1. Первое подтверждение...
2. Второе подтверждение...
3. Третье подтверждение..."""
...
<b>📝 Формат ответа:</b>
Сформулируйте <b>по одному подтверждению</b> для каждой характеристики на основе Конституции РФ.

Отправьте ответ в виде нумерованного списка:
1. Подтверждение первой характеристики...
2. Подтверждение второй характеристики...
3. Подтверждение третьей характеристики..."""
